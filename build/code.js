...
async function exportAsset(req) {
    const frame = await buildResizedFrame(req);
    try {
        const settings = req.format === "SVG"
            ? { format: "SVG" }
...
function classifyFrame(frame) {
    ...
    const assets = kids.filter((k) => k !== background && k.type !== "TEXT");
 ...
 *   - text untouched
 */
function applyContextResize(frame, W, H, threshold, anchorH, margin) {
